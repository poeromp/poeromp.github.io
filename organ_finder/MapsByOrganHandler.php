<?php
    $organ = (isset($_POST['organ']) && $_POST['organ'] != '') ? $_POST['organ'] : '';
    $limit = (isset($_POST['mapLimit']) && $_POST['mapLimit'] != '') ? $_POST['mapLimit'] : 1;
    ...
    if($organ != '') {
        // organ exists
        if (array_key_exists($organ, $organToMap)) {
            $maps = $organToMap[$organ];
            ...
            while(!$mapAdded) {
                ...
                foreach ($maps as $map) {
                    // check if organ drops of this map are smaller than or equal to the specified limit
                    if (($mapOrganCount = count($mapToOrgan[$map])) <= $limit) {
                        // append map to return string
                        $mapCollection[] = [
                            'map' => $map,
                            'mapOrganCount' => $mapOrganCount,
                            'organs' => implode(', ', $mapToOrgan[$map]),
                        ];

                        // mark while loop as completed
                        $mapAdded = true;
                    }
                }

                // if no maps have been found, increase organ drop limit
                $limit++;
            }

            // build map string
            sort($mapCollection);
            echo json_encode($mapCollection);
        }
        else {
